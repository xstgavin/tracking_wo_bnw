...
def draw_track_path(frame,bbox,tid):
    ...
    cv2.putText(frame,'%03d'%tid,pt0,cv2.FONT_HERSHEY_SIMPLEX,0.5,color[str(tid+1)],2)
    return frame
...
def get_labels(path, extension='pysot_crt',fmt='old'):
...
def test():
    get_labels('../../data_real/videos/',fmt='new')
